...
import random, copy, os, math, numpy, copy, time
import utils
...
class z_expander(nn.Module):
    def __init__(self, opt, n_steps):
        ...
        self.z_expander = nn.Sequential(
            ...
            nn.Linear(
                opt.nfeature,
                n_steps * opt.nfeature * self.opt.h_height * self.opt.h_width,
            ),
        )
...
class z_network_gaussian(nn.Module):
    def __init__(self, opt):
        ...
        self.network = nn.Sequential(
            nn.Linear(
                opt.nfeature * self.opt.h_height * self.opt.h_width,
                opt.nfeature,
            ),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Linear(opt.nfeature, opt.nfeature),
...
class PriorMDN(nn.Module):
    ...
    def forward(self, input_images, input_states):
        ...
        sigma = F.softplus(self.sigma_net(h)).view(
            bsize, self.opt.n_mixture, self.opt.nz
        )
...
class PriorGaussian(nn.Module):
    ...
    def forward(
        ...
        if normalize_inputs:
            ...
            input_states -= (
                self.stats["s_mean"].view(1, 4).expand(input_states.size())
            )
            input_states /= (
                self.stats["s_std"].view(1, 4).expand(input_states.size())
            )
...
class v_network_mdn_fc(nn.Module):
    ...
    def forward(self, h):
        ...
        sigma = F.softplus(self.sigma_net(h)).view(
            bsize, self.opt.n_mixture, self.n_outputs
        )
        sigma = torch.clamp(sigma, min=1e-3)
        return pi, mu, sigma
...
class policy_encoder(nn.Module):
    ...
    def forward(self, state_images, states):
        ...
        state_images = state_images.view(
            bsize,
            self.n_channels * self.opt.ncond,
            self.opt.height,
            self.opt.width,
        )
...
class FwdCNN_VAE(nn.Module):
    ...
    def sample_z(self, bsize, method=None, h_x=None):
        if self.opt.model == "fwd-cnn-vae-fp":
            ...
            mu_logvar_prior = self.z_network_prior(h_x.view(bsize, -1)).view(
                bsize, 2, self.opt.nz
            )
...
class StochasticPolicy(nn.Module):
    ...
    def forward(
        ...
        std = logvar.mul(0.5).exp_()
        eps = torch.randn(
            bsize, n_samples, self.n_outputs
        ).cuda()  # .cuda() is FUCKING wrong!
...
class DeterministicPolicy(nn.Module):
    def __init__(self, opt, context_dim=0, output_dim=None, n_channels=4):
        ...
        self.encoder = encoder(
            opt, a_size=0, n_inputs=opt.ncond, n_channels=n_channels
        )
        self.n_outputs = opt.n_actions if output_dim is None else output_dim
        self.hsize = opt.nfeature * self.opt.h_height * self.opt.h_width
        self.proj = nn.Linear(self.hsize, opt.n_hidden)
    ...
    def forward(
        ...
        return (
            ...
            None,
            None,
        )  # Returning a tuple of 4, for consistency with the stochastic policy
...
class PolicyMDN(nn.Module):
    def __init__(self, opt, n_mixture=10, npred=1):
        ...
        self.sigma_net = nn.Linear(
            opt.n_hidden, opt.n_mixture * self.n_outputs
        )
    ...
    def forward(
        ...
        if sample:
            ...
            for b in range(bsize):
                ...
                a.append(
                    torch.randn(self.npred, self.opt.n_actions).cuda()
