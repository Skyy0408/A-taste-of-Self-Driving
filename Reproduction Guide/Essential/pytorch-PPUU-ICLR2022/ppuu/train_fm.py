...
def main(config):
    ...
    trainer = pl.Trainer(
        ...
        strategy=config.training.distributed_backend if config.training.distributed_backend else "auto",
        default_root_dir=logger.first_log_dir,
        # track_grad_norm=2, # Deprecated or removed?
    )
    ...
    trainer.fit(model, data_module, ckpt_path=config.training.resume_from_checkpoint)
    return model
