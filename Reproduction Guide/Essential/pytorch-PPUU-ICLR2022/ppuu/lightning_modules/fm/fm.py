...
class FM(pl.LightningModule):
    ...
    def __init__(
        ...
        if self.config.model.fm_type == 'vanilla':
        ...
        self.model = get_fm(self.config.model.fm_type)(
            ...
            predict_state=self.config.model.predict_state,
        ...
        if self.config.model.checkpoint is not None:
            checkpoint = torch.load(self.config.model.checkpoint, weights_only=False)
    ...
    def unfold(self, *args, **kwargs):
        return self.model.unfold(*args, **kwargs)
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.Adam(
            ...
            * self.config.training.gpus
            * self.config.training.num_nodes
            * np.sqrt(self.config.training.batch_size / 64),
        )
