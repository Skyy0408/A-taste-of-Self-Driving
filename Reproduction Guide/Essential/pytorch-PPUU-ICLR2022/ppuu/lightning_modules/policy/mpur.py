    ...
    def wrapper(cls_):
        ...
        class Cls(cls_):
            ...
            class Config(configs.ConfigBase):
                model: cls_.ModelConfig = field(default_factory=cls_.ModelConfig)
                cost: cost_type.Config = field(default_factory=cost_type.Config)
                training: cls_.TrainingConfig = field(default_factory=cls_.TrainingConfig)
...
class MPURModule(pl.LightningModule):
    ...
    def __init__(self, hparams=None):
        ...
        if os.path.exists(self.config.model.forward_model_path):
            # If the above is false, we may be just using lightning
            # to load the weights.
            ...
            if hasattr(self.forward_model, "state_predictor"):
                assert self.forward_model.state_predictor.diff == self.config.training.diffs
        ...
        self.policy_model = policy_models.DeterministicPolicy(
            ...
            enc_layers=self.config.model.enc_layers,
        )
    ...
    def forward(self, batch: DatasetSample) -> FwdCNN_VAE.Unfolding:
    ...
    def configure_optimizers(self):
        optimizer = optim.Adam(
            ...
            * self.config.training.gpus
            * self.config.training.num_nodes
            * (self.config.training.batch_size / 6),
        )
    ...
    def _setup_policy_cost(self):
        ...
        self.policy_cost.estimate_uncertainty_stats(self.trainer.datamodule.train_dataloader())
