...
class PolicyCostKMTaper(PolicyCost):
            ...
            for y in range(results.shape[1]):
                ...
                p_cost = (
                    self.compute_proximity_cost_km(
                        s_image.unsqueeze(0).unsqueeze(0).to(image.device),
                        masks,
                        mask_sums,
                    )
    ...
    def _adjust_state_grad(self, states: torch.Tensor) -> torch.Tensor:
        return (
            states.detach() * (1 - self.config.state_grad_coeff)
            + states * self.config.state_grad_coeff
        )
    ...
    def compute_state_costs_for_training(
        ...
        proximity_mask, proximity_mask_lo = self.get_masks(
            self._adjust_state_grad(scalar_states),
            actions,
            context_state_seq,
            unnormalize=True,
        )
    ...
    def store_visualization_data(self, state_seq):
        self._last_traj_states = state_seq.states.detach().cpu()

    def get_last_cost_profile_and_traj(self):
        return (
            self._last_cost_profile_and_traj,
            self._last_cost_profile,
            getattr(self, '_last_traj_states', None),
            getattr(self, '_last_masks', None)
        )
    ...
    def _build_cost_profile_and_traj(
        ...
        if (
            ...
            cost_landscape_unnormed = self.get_cost_landscape(
                last_image[0, 0].to(scalar_states.device),
                proximity_mask[0, 0].unsqueeze(0).unsqueeze(0),
                proximity_mask_sum[0, 0].unsqueeze(0).unsqueeze(0),
            )
            cost_landscape = cost_landscape_unnormed / (
            ...
            self._last_cost_profile_and_traj = (
                cost_landscape.to(scalar_states.device) + traj_proximity_mask.sum(dim=1)[0]
            ).contiguous()
            self._last_cost_profile = (
