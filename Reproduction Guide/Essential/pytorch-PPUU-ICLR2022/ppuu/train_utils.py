...
class CustomLoggerWB(pl.loggers.WandbLogger):
    def __init__(
        ...
        **kwargs,
        ...
        if save_dir is not None:
            self.custom_log_dir = os.path.join(save_dir, experiment_name, seed)

            name = experiment_name

            # find first free directory
            self.first_log_dir = self.custom_log_dir
            if version is not None:
                ...
                self.custom_log_dir = f"{self.custom_log_dir}_{version}"
            else:
                if os.path.exists(self.custom_log_dir):
                    k = 1
                    while os.path.exists(f"{self.custom_log_dir}_{k}"):
                        k += 1
                    self.custom_log_dir = f"{self.custom_log_dir}_{k}"
                    name = f"{name}_{k}"

            os.makedirs(self.custom_log_dir, exist_ok=True)
    ...
    def save(self):
        super().save()
        if self.custom_log_dir is not None:
            os.makedirs(self.custom_log_dir, exist_ok=True)
            logs_json_save_path = os.path.join(
                self.custom_log_dir, self.json_filename
            )
    ...
    def log_hyperparams(self, hparams):
        ...
        if self.custom_log_dir is not None:
            os.makedirs(self.custom_log_dir, exist_ok=True)
            self.config_path = os.path.join(self.custom_log_dir, "config.yaml")
            if not os.path.exists(self.config_path):
                with open(self.config_path, "w") as yaml_file:
                    yaml.dump(
                        dict(hparams), yaml_file, default_flow_style=False
                    )
