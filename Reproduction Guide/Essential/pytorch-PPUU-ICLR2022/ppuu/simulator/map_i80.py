...
class I80Car(Car):
    ...
    def policy(self, *args, **kwargs):
        ...
        if self.fucked_up:
            # if the car is off screen, it just gets stuck.
            # Nothing moving after that.
            print('a car is off screen and is frozen')
            return np.array((0, 0))

        if self.env_policy_model is not None and len(self._states_image) > 20:
            # if len(self._states_image) > 20:
            transpose = list(zip(*self._states_image))
            state_images = transpose[0]
            state_images = torch.stack(state_images).permute(0, 3, 1, 2)[-20:]

            ego_car_channel = self._ego_car_image[:, :]
            ...
            conditional_sequence = (
                StateSequence(
                    state_images.unsqueeze(0),
                    states.unsqueeze(0),
                    torch.tensor([self._width, self._length]).unsqueeze(
                        0
                    ),  # car size
                    self._ego_car_image.permute(2, 0, 1).unsqueeze(0),
                )
                .with_ego()
                .cuda()
            )

            action = self.env_policy_model(
                conditional_sequence,
                normalize_inputs=True,
                normalize_outputs=True,
            ).flatten()
            assert action.shape == (2,), 'action shape is not (2,)'
            a, b = action[0].item(), action[1].item()
        else:  # We just use the recorded trajectory if policy is not avaiable.

            new_speed = self._get("speed", self._frame)
            a = (new_speed - self._speed) / self._dt

            ortho_direction = np.array(
...
class I80(Simulator):
    ...
    def __init__(self, **kwargs):
        ...
        self.data_stats = (
            torch.load(pth, map_location='cpu', weights_only=False)
    ...
    def _get_data_frame(self, time_slot, x_max, x_offset):
        ...
        file_name = f"{self.data_dir}/../../xy-trajectories/{time_slot}"  # noqa: E501
        ...
        if vehicles:
            ...
            for vehicle_id in vehicles:
                ...
                car = self.EnvCar(
                    ...
                    dt=self.delta_t,
                    env_policy_model=self.env_policy_model,
                )
