...
class Car:
    ...
    def _get_observation_image(
        ...
        try:
            ...
            self.fucked_up = True  # we're off_screen, but different
...
class Simulator(core.Env):
    ...
    def __getstate__(self):
        ...
        # Remove unpicklable pygame objects
        state['screen'] = None
        state['font'] = None
        state['clock'] = None
        state['_lane_surfaces'] = dict()
        # state['random'] = None # Random state should probably be kept or handled carefully
        return state

    def __setstate__(self, state):
        self.__dict__.update(state)
        # Restore unpicklable objects if needed (though deepcopy usually doesn't need them immediately)
        # If display is True, we might need to re-init pygame, but for deepcopy in evaluator it's likely fine.
        if self.display and self.screen is None:
             # We cannot easily restore screen/pygame context in a deepcopy
             # But usually deepcopy is for rollback, so we might not need display there.
             pass
