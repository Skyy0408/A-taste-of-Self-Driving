...
class MPCKMPolicy(torch.nn.Module):
    ...
    class Config(configs.ConfigBase):
        ...
        lambda_action_barrier: float = 1.0
        lbfgs_max_iter: int = 10
        actions_limit: float = 3.0
    ...
    def _action_barrier(self, actions: torch.Tensor) -> torch.Tensor:
        """actions should be normalized when passed to this function"""

        if self.config.old_barrier:
            barrier = (
                (torch.clamp(actions.abs() - 10, min=0) ** 2)
                .mean(dim=-1)
                .mean(dim=-1)
            )
            ...
            actions_unnormed = self.normalizer.unnormalize_actions(actions)
            barrier = (
                (
                    torch.clamp(
                        actions_unnormed.abs() - self.config.actions_limit,
                        min=0,
                    )
                )
                .mean(dim=-1)
    ...
    def _repeat_actions_for_horizon(self, actions: torch.Tensor, dim=1):
        ...
        if actions.shape[dim] != self.config.unfold_len:
            ...
            ), f"can't use specified actions size ({actions.shape[dim]}) with horizon={self.config.unfold_len}"
    ...
    def get_cost(
        ...
        loss_a_barrier = (
            self._action_barrier(flatten_2_dims(actions))
            .view(batch_size, action_batch_size)
            .unsqueeze(1)
        )
        ...
        result = (
            ...
            + loss_a_barrier
            * self.config.lambda_action_barrier  # shape bsize, 1, action_batch
        )
    ...
    def _ce_optimize(
        ...
        # Uncertainty Visualization: Unfold multiple samples for the best action
        if self.visualizer is not None and self.config.use_fm:
            with torch.no_grad():
                # Use a larger number of samples for visualization if needed, or use config
                n_samples = 10 # or self.config.fm_unfold_samples
                
                # We need to temporarily override the aggregation to 'keep' to get all samples
                original_agg = self.config.fm_unfold_samples_agg
                original_samples = self.config.fm_unfold_samples
                
                self.config.fm_unfold_samples_agg = "keep"
                self.config.fm_unfold_samples = n_samples
                
                # Unfold
                # actions is [batch, plan_len, 2] -> need [batch, unfold_len, 2]
                # self.last_actions is already repeated to unfold_len
                
                vis_seq = self.unfold_fm(
                    conditional_state_seq,
                    self.last_actions,
                    Z=None # Let it sample Z
                )
                
                # Restore config
                self.config.fm_unfold_samples_agg = original_agg
                self.config.fm_unfold_samples = original_samples
                
                # Store in cost for visualization
                if hasattr(self.cost, "store_visualization_data"):
                    self.cost.store_visualization_data(vis_seq)
...
class MPCFMPolicy(MPCKMPolicy):
    def plan(
        self,
        conditional_state_seq: StateSequence,
        _gt_future_seq=None,  # ignored
        metadata: Optional[dict] = None,
        full_plan: bool = False,
    ) -> torch.Tensor:
        """Plans given conditional state sequence.
        Everything is normalized in input and output.
        """
        ...
        # Keep Z constant to make optimization easier.
        Z = (
            torch.randn(*actions[:, 0].shape[:3], 32)
            * self.config.fm_unfold_variance
        ).to(actions.device)

        for i in range(self.config.n_iter):
            pred_state_seq = self.unfold_fm(
                conditional_state_seq,
                actions[:, 0],
                Z=Z,
            )
            ...
            costs = self.cost.calculate_cost(
                conditional_state_seq.map(flatten_2_dims),
                actions[:, 0],
                pred_state_seq.map(flatten_2_dims),
            )
        ...
        best_actions = actions[:, 0]  # remove actions_batch_size
